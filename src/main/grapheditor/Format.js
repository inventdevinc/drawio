...
ArrangePanel.prototype.init = function()
{
	...
	this.container.appendChild(this.addIDOps(this.createPanel()));
 ...
 * 
 */
ArrangePanel.prototype.addIDOps = function(div)
{
	var ui = this.editorUi;
	var graph = ui.editor.graph;
	var cell = graph.getSelectionCell();
	...
	div.appendChild(this.createTitle('Invent Dev Actions'));
	if (graph.getSelectionCount() == 1)
	{
		if (count > 0)
		{
		...
		btn = mxUtils.button('Edit Finish Data', mxUtils.bind(this, function(evt)
		{
			this.editorUi.actions.get('editFinish').funct();
		}));

		// btn.setAttribute('title', 'Edit as finish' + ' (' + this.editorUi.actions.get('editData').shortcut + ')');
		btn.style.width = '202px';
		btn.style.marginBottom = '2px';
		div.appendChild(btn);
		count++;
		...
		btn = mxUtils.button('Edit Model Data', mxUtils.bind(this, function(evt)
		{
			this.editorUi.actions.get('editModel').funct();
		}));
